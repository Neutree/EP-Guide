...
bool Communicate::SendBytesToServer(esp8266 & wifi,char* ipAddress,uint32_t port,char* data,uint32_t len)
{
	...
	//wifi.CloseMulitpleSend(4);   这里不关闭，放在通信结束后关闭
	return false;
}
...
///@param 是否发送成功（对方不一定接收到，只是发送成功）
////////////////////////////
bool Communicate::ReturnBytesToClient(esp8266 & wifi,uint8_t linkID,char* data,uint32_t len)
{
	return wifi.sendMultipleMode(linkID,data,len);
}
